module.exports = {
    ...
    thisAppIs: [
        ...
        'Это приложение является продвинутым Soundboard\'ом, позволяющим не просто проигрывать звуки в микрофон, но и "на лету" транслировать текст, преобразованный в речь через GTTS, в указанный в настройках выход звука. Программа может проигрывать звуки как из файлов, так и по URL. Но URL должна быть на YouTube.com (MPV требует для этого youtube-dl) или являться прямой ссылкой на файл. Дистрибутив также включает некоторые бинды и shortcut\'ы в качестве примера, которые я посчитал наиболее нужными. Вы же можете все их удалить, естественно. На Linux их нужно вручную положить в "~/.config/f0talk".'
    ],
    isFor: [
