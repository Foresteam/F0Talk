...
class Command {
    ...
    printHelp() {
        ...
        for (let arg of this.args) {
            ...
            args.push(arg.name + (defval ? '=' + defval : '') + ': ' + type + (arg.desc ? ' - ' + (arg.desc instanceof Function ? arg.desc() : arg.desc) : ''));
        }
        return `${name}` + (alts.length > 1 ? `[${alts.join(', ')}]` : '') + `${args.length > 0 ? ' (' + args.join(', ') + ')' : ''} - ${this.help instanceof Function ? this.help() : this.help}`;
    }
}
    ...
    parse(msg, commands) {
        ...
        try {
            for (let arg of text.split(' ')) {
                ...
                getType();
                if (!doVArg && ttype == '.?' && arg.startsWith('.'))
                    val = arg.substr(1);
                else {
                    if (!doVArg && ttype == '.?') {
                        rargs[args[i].name] = args[i].default;
                        args.splice(i, 1);
                        getType();
                    }
